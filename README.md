...
会议日历入口为 `http://127.0.0.1:8000/calendar`，资料库入口为 `http://127.0.0.1:8000/archive`，备份中心为 `http://127.0.0.1:8000/backups`：
...
- 完整备份包含活跃、归档及回收站会议、任务历史和会议目录，不包含模型缓存或外部 LLM API Key。
- 恢复采用合并导入，相同会议 ID 或目录名会跳过，不覆盖当前数据；备份中的未完成任务会恢复为已取消状态，可按现有断点手动继续。
- SQLite 数据库使用显式版本迁移，当前 schema 为 v3；旧版数据库在启动时自动补齐任务表和会议生命周期字段。
